...
# 메뉴 바 객체를 생성합니다. 부모 창이나 부모 메뉴 바를 지정해야 합니다.
menubar = tk.Menu(window)  
window.config(menu=menubar)

# 파일 메뉴 객체를 생성합니다.
file_menu = tk.Menu(menubar, tearoff=0)
# 사용자가 클릭했을 때 특정 함수(command=함수명)를 실행하는 일반 버튼을 추가합니다.
...
# 마우스를 올리거나 클릭했을 때 하위 메뉴가 펼쳐지도록 메뉴와 메뉴를 연결합니다.
